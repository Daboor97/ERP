...
{
    public partial class customerDivisionDetails : UserControl
    {
        public customerDivisionDetails()
        {
            InitializeComponent();
            fill();
        ...
        private void showFlowButton_Click(object sender, EventArgs e)
        {
            ...
            string Query = "select debtsTable.debtorder as 'رقم القسط', debtsTable.debtAmount as 'قيمة القسط', debtsTable.debtDate as 'معاد الدفع' , debtsTable.paidAmount as 'القيمة المسددة' ,debtsTable.paidDate as 'تاريخ الدفع' ,debtsmainTable.billNumber as 'رقم الفاتورة' from debtsMainTable,debtsTable,salesMainTable where debtsMainTable.billNumber = salesMainTable.Id and debtsMainTable.Id = debtsTable.debtMainTableNumber and salesMainTable.customerName = N'" + this.customerNameComboBox.Text + "' and debtDate between '" + this.fromDate.Value.ToString("MM/dd/yyyy") + "' AND '" + this.toDate.Value.ToString("MM/dd/yyyy") + "' ;";
