...
{
    public partial class customerDivisonDate : UserControl
    {
        public customerDivisonDate()
        {
            InitializeComponent();
            fill();
        ...
        //TODO  مش شغالة صح الQuery 
        private void showFlowButton_Click(object sender, EventArgs e)
        {
            editedDGV.DataSource = null;
            editedDGV.Refresh();

            string Query = "select debtsTable.debtorder as 'رقم القسط', debtsTable.debtAmount as 'قيمة القسط', debtsTable.debtDate as 'معاد الدفع' , debtsTable.paidAmount as 'القيمة المسددة' ,debtsmainTable.billNumber as 'رقم الفاتورة' from debtsMainTable,debtsTable,salesMainTable where debtsMainTable.billNumber = salesMainTable.Id and debtsMainTable.Id = debtsTable.debtMainTableNumber and salesMainTable.customerName = N'" + this.customerNameComboBox.Text + "' and debtsTable.status = 'False' and debtDate between '" + this.fromDate.Value.ToString("MM/dd/yyyy") + "' AND '" + this.toDate.Value.ToString("MM/dd/yyyy") + "' ;";
