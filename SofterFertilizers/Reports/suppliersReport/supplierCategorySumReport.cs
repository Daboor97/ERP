...
{
    public partial class supplierCategorySumReport : UserControl
    {
        public supplierCategorySumReport()
        {
            InitializeComponent();
            fill();
        }
        ...
        private void showFlowButton_Click(object sender, EventArgs e)
        {
            ...
            string Query = "SELECT distinct categoryCode,unit  FROM purchasesSubTable,purchasesMainTable where purchasesSubTable.billCode = purchasesMainTable.Id and date between '" + this.fromDate.Value.ToString("MM/dd/yyyy") + "' AND '" + this.toDate.Value.ToString("MM/dd/yyyy") + "'  and supplierName=N'" + this.customerNameComboBox.Text + "'; ";
            SqlConnection conDataBase = new SqlConnection(constring);
            ...
            for (int i = 0; i <= editedDGV.Rows.Count - 1; i++)
            {
                ...
                conDataBase = new SqlConnection(constring);
                conDataBase.Open();
                string sum = new SqlCommand("select SUM(quantity) from purchasesSubTable,purchasesMainTable where purchasesSubTable.billCode = purchasesMainTable.Id and date between '" + this.fromDate.Value.ToString("MM/dd/yyyy") + "' AND '" + this.toDate.Value.ToString("MM/dd/yyyy") + "'  and supplierName=N'" + this.customerNameComboBox.Text + "'and categoryCode=N'" + Convert.ToInt32(editedDGV.Rows[i].Cells[0].Value) + "';", conDataBase).ExecuteScalar().ToString();
                conDataBase.Close();
