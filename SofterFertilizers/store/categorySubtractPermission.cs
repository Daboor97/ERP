...
{
    public partial class categorySubtractPermission : UserControl
    {
        string constring = System.Configuration.ConfigurationManager.ConnectionStrings["constring"].ConnectionString;

        public categorySubtractPermission()
        {
            InitializeComponent();
            clear();
            fillCategoryDGV();
        ...
        void fill()
        {
            ...
            conDataBase.Open();
            string maxBill = new SqlCommand("IF EXISTS (select MAX(Id) from permissionSubtractionMainTable) BEGIN SELECT MAX(Id) FROM permissionSubtractionMainTable END", conDataBase).ExecuteScalar().ToString();

            if (maxBill != "")
            {
                int maxBillint = Convert.ToInt32(maxBill);
                permissionSubtractCode.Text = Convert.ToString(maxBillint + 1);
                conDataBase.Close();
            }

            else
            {
                permissionSubtractCode.Text = "1";
                conDataBase.Close();
            }
        }
        ...
        private void saveCategoryButton_Click(object sender, EventArgs e)
        {
            try
            {
                string Query = "IF NOT EXISTS (select 1 FROM permissionSubtractionMainTable where Id= N'" + this.permissionSubtractCode.Text + "') BEGIN INSERT INTO permissionSubtractionMainTable(StoreName,sum,status,date) VALUES (N'" + this.storeNameComboBox.Text + "',N'" + this.totalSumTextBox.Text + "','False',N'" + this.dateDTP.Value.ToString("MM/dd/yyyy") + "') END ";
                SqlConnection conDataBase = new SqlConnection(constring);
                SqlCommand cmdDataBase = new SqlCommand(Query, conDataBase);
