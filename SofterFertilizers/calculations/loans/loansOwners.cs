...
{
    public partial class loansOwners : UserControl
    {
        public loansOwners()
        {
            InitializeComponent();
        ...
        void fill()
        {
            ...
            conDataBase.Open();
            string maxBill = new SqlCommand("IF EXISTS (select MAX(Id) from loansMainTable) BEGIN SELECT MAX(Id) FROM loansMainTable END", conDataBase).ExecuteScalar().ToString();
        ...
        private void savedDivisionButton_Click(object sender, EventArgs e)
        {
           string Query = "INSERT INTO loansMainTable (name,telephone,mobile,fax,amount,debtsNumber,debtAmount,time,timeType,startDate,endDate) VALUES (N'" + this.nameTextBox.Text + "',N'" + this.telephoneTextBox.Text + "',N'" + this.mobileTextBox.Text + "',N'" + this.faxTextBox.Text + "',N'" + this.restDebtTextBox.Text + "',N'" + this.DebtsNumberTextBox.Text + "',N'" + this.debtAmountTextBox.Text + "',N'" + this.debtTimeValueTextBox.Text + "',N'" + debtTime + "',N'" + this.startDebtDate.Value.ToString("MM/dd/yyyy") + "',N'" + this.endDebtDateDGV.Text + "') ;";
            ...
            conDataBase.Open();
            string debtId = new SqlCommand("SELECT Id FROM loansMainTable WHERE name = N'" + this.nameTextBox.Text + "' AND telephone = N'" + this.telephoneTextBox.Text + "' AND mobile = N'" + this.mobileTextBox.Text + "'  AND fax = N'" + this.faxTextBox.Text + "' AND amount = N'" + this.restDebtTextBox.Text + "' AND debtsNumber = N'" + this.DebtsNumberTextBox.Text + "' AND debtAmount = N'" + this.debtAmountTextBox.Text + "'AND time = N'" + this.debtTimeValueTextBox.Text + "' AND timeType = N'" + debtTime + "'", conDataBase).ExecuteScalar().ToString();
            conDataBase.Close();


            for (int i = 0; i < debtGrid.Rows.Count; i++)
            {
                Query = "INSERT INTO loansTable (debtOrder,debtAmount,debtDate,status,paidDate,paidAmount,loanMainTableNumber) VALUES (N'" + this.debtGrid.Rows[i].Cells[0].Value + "',N'" + this.debtGrid.Rows[i].Cells[1].Value + "',N'" + this.debtGrid.Rows[i].Cells[2].Value + "','False',GETDATE(),'0',N'" + debtId + "') ;";
                conDataBase = new SqlConnection(constring);
