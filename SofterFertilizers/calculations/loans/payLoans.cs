...
{
    public partial class payLoans : UserControl
    {
        public payLoans()
        {
            InitializeComponent();
        ...
        private void loanNumberComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string Query = "select debtorder as 'رقم القسط'  ,debtAmount as 'قيمة القسط', debtDate as 'تاريخ القسط',status as 'الحالة',paidAmount as 'قيمة المدفوع', paidDate as 'تاريخ الدفع' from loansTable where loanMainTableNumber =N'"+this.loanNumberComboBox.Text+"' ;";
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            if (adjustCheckBox.Checked)
            {
                ...
                {
                    string Query = "UPDATE loansTable SET status ='TRUE', paidDate=N'" + this.dateDTP.Value.ToString("MM/dd/yyyy") + "', paidAmount=N'" + this.totalPaidTextBox.Text + "'  where debtorder=N'" + this.divisionNumber.Text + "' and loanMainTableNumber=N'"+this.loanNumberComboBox.Text+"' ";
                    SqlConnection conDataBase = new SqlConnection(constring);
                    SqlCommand cmdDataBase = new SqlCommand(Query, conDataBase);
                    ...
                    Query = "DELETE FROM safeTable where billNo = N'" + loanNumberComboBox.Text + "' and type='loanPay' and details ='out' and notes = N'قسط رقم " + divisionNumber.Text + "' ;";
                    conDataBase = new SqlConnection(constring);
                    ...
                    //الخزنة
                    Query = "INSERT INTO safeTable(name,notes,money,type,date,details,billNo,paymentType,clientCode) VALUES (N'" + this.safeComboBox.Text + "' ,N'قسط رقم " + divisionNumber.Text + "',N'" + this.paidAmount.Text + "','loanPay',N'" + this.dateDTP.Value.ToString("MM/dd/yyyy") + "','out',N'" + this.loanNumberComboBox.Text + "','cash','')";
                    conDataBase = new SqlConnection(constring);
                    cmdDataBase = new SqlCommand(Query, conDataBase);
                    try
                    {
                ...
                {
                    string Query = "UPDATE loansTable Set status ='False', paidDate=N'" + this.dateDTP.Value.ToString("MM/dd/yyyy") + "', paidAmount=N'" + this.totalPaidTextBox.Text + "'  where debtorder=N'" + this.divisionNumber.Text + "' and loanMainTableNumber= N'" + this.loanNumberComboBox.Text + "'";
                    SqlConnection conDataBase = new SqlConnection(constring);
                    SqlCommand cmdDataBase = new SqlCommand(Query, conDataBase);
                    SqlDataReader myReader;
                    try
            ...
            else
            {
                ...
                else if (Convert.ToInt32(divisionAmount.Text) > Convert.ToInt32(totalPaidTextBox.Text))
                {
                    ...
                    Query = "INSERT INTO safeTable(name,notes,money,type,date,details,billNo,paymentType,clientCode) VALUES (N'" + this.safeComboBox.Text + "' ,N'قسط رقم " + divisionNumber.Text + "',N'" + this.paidAmount.Text + "','loanMainTableNumber',N'" + this.dateDTP.Value.ToString("MM/dd/yyyy") + "','out',N'" + this.loanNumberComboBox.Text + "','cash','')";
                    conDataBase = new SqlConnection(constring);
                    cmdDataBase = new SqlCommand(Query, conDataBase);
                    try
                    {
