...
{
    public partial class loansDetails : UserControl
    {
        public loansDetails()
        {
            InitializeComponent();
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection conDataBase = new SqlConnection(constring);
            conDataBase.Open();
            string stringStatus = new SqlCommand("IF EXISTS(select 1 from loansTable where loanMainTableNumber = N'" + oldBill + "' and status='True') SELECT 0 ELSE SELECT 1 ", conDataBase).ExecuteScalar().ToString();
            conDataBase.Close();
            ...
            if (status)
            {
                ...
                string Query = "IF NOT EXISTS (select 1 from loansTable where loanMainTableNumber = N'" + oldBill + "' and status='True') BEGIN DELETE FROM loansTable where loanMainTableNumber = N'" + oldBill + "' END ";
                ...
                Query = "IF NOT EXISTS (select 1 from loansTable where  loanMainTableNumber= N'" + oldBill + "' and status='True') BEGIN DELETE FROM loansMainTable where Id = N'" + oldBill + "' END ";
                conDataBase = new SqlConnection(constring);
