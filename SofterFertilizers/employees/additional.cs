...
namespace SofterFertilizers.employees
{
    public partial class additional : UserControl
    {
        ...
        private void employeeNameComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string Query = "select employeeName as 'اسم الموظّف' ,hours as 'ساعات الإضافية', minutes as 'الدقائق الإضافية' , date as 'التاريخ' ,outside as 'لا يمكن تعديله' from employeeAdditionalTable where employeeName = N'" + this.employeeNameComboBox.Text + "' ; ";
        ...
        private void saveButton_Click(object sender, EventArgs e)
        {
            if (status == "new")
            {
                string Query = "INSERT INTO employeeAdditionalTable(employeeName,hours,minutes,date,outside) VALUES (N'" + this.employeeNameComboBox.Text + "',N'" + this.hoursTextBox.Text + "',N'" + this.minutesTextBox.Text + "',N'" + this.toDate.Value.ToString("MM/dd/yyyy") + "','False')  ";
                SqlConnection conDataBase = new SqlConnection(constring);
                SqlCommand cmdDataBase = new SqlCommand(Query, conDataBase);
            ...
            else
            {
                ...
                //supplier Code
                conDataBase.Open();
                string Outside = new SqlCommand("select outside from employeeAdditionalTable where employeeName =N'" + this.employeeNameComboBox.Text + "' and hours =N'" + this.oldHours + "' and minutes =N'" + this.oldMinutes + "' and  date =N'" + this.oldDate + "';", conDataBase).ExecuteScalar().ToString();
                conDataBase.Close();

                if (Convert.ToBoolean(Outside) == false)
                {
                    string Query = "BEGIN UPDATE employeeAdditionalTable SET date= N'" + this.toDate.Value.ToString("MM/dd/yyyy") + "', hours= N'" + this.hoursTextBox.Text + "', minutes= N'" + this.minutesTextBox.Text + "' where employeeName =N'" + this.employeeNameComboBox.Text + "' and hours =N'" + this.oldHours + "' and minutes =N'" + this.oldMinutes + "' and  date =N'" + this.oldDate + "' END";
                    conDataBase = new SqlConnection(constring);
                    SqlCommand cmdDataBase = new SqlCommand(Query, conDataBase);
                    SqlDataReader myReader;
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            ...
            if (Convert.ToBoolean(Outside) == false)
            {
                ...
                {
                    string Query = "DELETE FROM employeeAdditionalTable where employeeName =N'" + this.employeeNameComboBox.Text + "' and hours =N'" + this.oldHours + "' and minutes =N'" + this.oldMinutes + "' and  date =N'" + this.oldDate + "' ;";
                    conDataBase = new SqlConnection(constring);
                    SqlCommand cmdDataBase = new SqlCommand(Query, conDataBase);
                    SqlDataReader myReader;
