...
{
    public partial class additionalReport : UserControl
    {
        ...
        private void showFlowButton_Click(object sender, EventArgs e)
        {
            ...
            if (employeeCodeTextBox.Text == "0")
            {
                string Query = "select employeeName as 'اسم الموظّف' ,hours as 'ساعات التأخير', minutes as 'دقائق التأخير' ,date as 'الاريخ', outside as 'من تسجيل الغياب والحضور' from employeeAdditionalTable where date between '" + this.fromDate.Value.ToString("MM/dd/yyyy") + "' AND '" + this.toDate.Value.ToString("MM/dd/yyyy") + "' ; ";
            ...
            {
                string Query = "select employeeName as 'اسم الموظّف' ,hours as 'ساعات التأخير', minutes as 'دقائق التأخير' ,date as 'الاريخ', outside as 'من تسجيل الغياب والحضور' from employeeAdditionalTable where employeeName = N'" + this.employeeNameComboBox.Text + "' and date between '" + this.fromDate.Value.ToString("MM/dd/yyyy") + "' AND '" + this.toDate.Value.ToString("MM/dd/yyyy") + "' ; ";
