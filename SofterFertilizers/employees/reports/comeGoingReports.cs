...
{
    public partial class comeGoingReports : UserControl
    {
        public comeGoingReports()
        {
            InitializeComponent();
            fill();
        ...
        private void showFlowButton_Click(object sender, EventArgs e)
        {
            ...
            if (employeeCodeTextBox.Text == "0")
            {
                string Query = "select employeeName as 'اسم الموظّف' ,date as 'التاريخ', come as 'الحضور' ,go as 'الانصراف', permission as 'بإذن' from employeeComeGoingTable where date between '" + this.fromDate.Value.ToString("MM/dd/yyyy") + "' AND '" + this.toDate.Value.ToString("MM/dd/yyyy") + "' ; ";
            ...
            {
                string Query = "select employeeName as 'اسم الموظّف' ,date as 'التاريخ', come as 'الحضور' ,go as 'الانصراف', permission as 'بإذن' from employeeComeGoingTable where employeeName = N'" + this.employeeNameComboBox.Text + "' and date between '" + this.fromDate.Value.ToString("MM/dd/yyyy") + "' AND '" + this.toDate.Value.ToString("MM/dd/yyyy") + "' ; ";
